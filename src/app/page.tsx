...
const VolkEquipmentSite = () => {
  ...
  const statsData = content?.homepage?.stats || [
    { number: "200,000+", label: "Units Delivered Globally" },
    { number: "15+", label: "Years Excellence" },
    { number: "24/7", label: "Tech Support" },
    { number: "98%", label: "Satisfaction Rate" }
  ];
  const stats = [
    { ...statsData[0], icon: <Package className="w-6 h-6" /> },
    { ...statsData[1], icon: <Award className="w-6 h-6" /> },
    { ...statsData[2], icon: <Clock className="w-6 h-6" /> },
    { ...statsData[3], icon: <TrendingUp className="w-6 h-6" /> }
  ];

  const advantagesData = content?.homepage?.advantages || [
    { icon: "Zap", title: "Kubota Power Plant", description: "Every machine features legendary Kubota diesel engines—proven for fuel efficiency, longevity, and reliable performance in the toughest PNW conditions." },
    { icon: "Wrench", title: "Advanced Hydraulics", description: "Superior hydraulic systems deliver exceptional digging force and smooth operation. Hidden gear motors reduce maintenance and extend machine life." },
    { icon: "Shield", title: "Built to Last", description: "Reinforced cast swing booms, telescopic expandable tracks, and heavy-duty construction ensure your Rippa excavator works for decades, not just years." },
    { icon: "Users", title: "Local PNW Support", description: "Based in Sammamish with 48-hour response times. We understand Pacific Northwest terrain and provide dedicated service throughout WA, OR, and ID." }
  ];
  ...
  const advantages = advantagesData.map((adv: any) => ({
    icon: iconMap[adv.icon] || <Zap className="w-10 h-10" />,
    title: adv.title,
    desc: adv.description || adv.desc
  }));

  const industries = content?.homepage?.solutions?.industries || [
  ...
  const hero = content?.homepage?.hero || {
    badge: "OFFICIAL RIPPA DEALER • PACIFIC NORTHWEST",
    title: "Premium Kubota-Powered",
    titleHighlight: "Mini Excavators",
    subtitle: "Built by Rippa. Backed by Volk Equipment. Engineered for the demanding terrain and projects of the Pacific Northwest."
  };
  
  const equipmentSection = content?.homepage?.equipment || {
    badge: "RIPPA MINI EXCAVATORS",
    title: "Find Your",
    titleHighlight: "Perfect Match",
    subtitle: "From compact 1-ton landscapers to powerful 3-ton excavators—all featuring Kubota diesel reliability"
  };
  
  const solutionsSection = content?.homepage?.solutions || {
    title: "Built for",
    titleHighlight: "Every Industry",
    subtitle: "Specialized solutions for your specific needs"
  };
  
  const ctaSection = content?.homepage?.cta || {
    title: "Ready to Power Up Your Fleet?",
    subtitle: "Get expert guidance, transparent pricing, and equipment that delivers results every single day.",
    features: ["No-pressure consultations", "Transparent fixed pricing", "Same-day quotes", "Local PNW service"]
  };
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-5xl mx-auto">
            <div className="inline-flex items-center gap-2 mb-6 px-5 py-2.5 bg-gradient-to-r from-cyan-500/20 to-blue-600/20 border border-cyan-500/30 rounded-full">
              ...
              <span className="text-cyan-400 text-sm font-bold tracking-wide">{hero.badge}</span>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-4">
              {equipmentSection.title} <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-blue-600">{equipmentSection.titleHighlight}</span>
            </h2>
      ...
      <section id="solutions" className="py-24 bg-gradient-to-b from-white to-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-4">
              {solutionsSection.title} <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-blue-600">{solutionsSection.titleHighlight}</span>
            </h2>
            <p className="text-xl text-slate-600">{solutionsSection.subtitle}</p>
          </div>
      ...
      <section id="contact" className="py-24 bg-slate-900 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-8 text-sm text-slate-400">
            {ctaSection.features.map((feature: string, idx: number) => (
